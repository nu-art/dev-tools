		...
		for folder in "${folders[@]}"; do
			...
			if [[ "${ts_watch}" ]]; then
				...
				if [[ -e "./src/${folder}/tsconfig.json" ]]; then
					...
					echo "bash ${Path_RootRunningDir}/relaunch-backend.sh ${absoluteSourcesFolder} ${absoluteOutputDir} ${folderName} ${Path_RootRunningDir}"
					local command="bash ${Path_RootRunningDir}/relaunch-backend.sh ${absoluteSourcesFolder} ${absoluteOutputDir} ${folderName} ${Path_RootRunningDir}"
